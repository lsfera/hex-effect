...
Effect.catchAllDefect(effect, (defect) =>
  Effect.fail(new InfrastructureError({ cause: defect }))
)
```
...
**After:**
```typescript
yield* Effect.tryPromise(() => sdk.batch(writes)).pipe(
...
```typescript
yield* Effect.tryPromise(() => sdk.batch(writes)).pipe(
  Effect.mapError((e) => classifyError(e.error))
)
```
...
Effect.provide(program, Layer.setConfigProvider(
